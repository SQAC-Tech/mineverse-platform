...
import { upgradeStructure, StructureType } from '@/lib/gameplay/structures/service';
import { z } from 'zod';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = upgradeSchema.safeParse({ ...body, idempotency_key: body.idempotency_key || idempotencyKeyHeader });
    ...
    const res = await upgradeStructure(session.team_id, round_id, type as StructureType, idempotency_key);
